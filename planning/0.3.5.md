...
`WU-00` through `WU-18` are complete. Tool `0.3.5` implementation-completion verification has passed, but `WU-19` remains a mandatory pre-merge user-facing distribution-guidance task. Explicit release preparation remains a separate boundary and `releasenote/0.3.5.md` is still deferred.
