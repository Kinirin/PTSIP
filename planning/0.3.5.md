...
| `WU-03` | `S2` | Registry and definition validation in `registry.py` | `WU-02` | valid/invalid definitions and unresolved references produce deterministic machine-readable diagnostics | `READY` | — |
| `WU-04` | `S2` | Framework-neutral runner and normalized result contract in `runner.py` | `WU-02` | runner result preserves case/purpose/target identity and cannot mutate PTSIP classification state | `READY` | — |
...
```text
WU-03  registry / definition validation
WU-04  runner / normalized result contract
...
`WU-03`, `WU-04`, and `WU-05` became eligible when `WU-02` completed. `WU-15` remains eligible from `WU-01`; `WU-06` and `WU-19` remain independent roots. Their relative order is not an architectural dependency.
