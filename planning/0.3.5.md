...
The initial package SHOULD be deliberately small but responsibility-complete. A flat list of unrelated VPMS modules is not the target. Physical package ownership must expose the major responsibility boundaries so later work does not accumulate at `src/vpms/` root.

The responsibility-based target structure is:
...
- `domain/model.py` — canonical VPMS data types and validation-independent semantic identities;
- `domain/registry.py` — deterministic loading, indexing, reference resolution, and structural validation of VPMS definitions;
- `domain/selector.py` — purpose-based Verification Case selection without relying on directory-name heuristics;
- `execution/runner.py` — framework-neutral execution orchestration and normalized case-level results;
- `execution/adapters/command.py` — the initial generic command runner so VPMS is not coupled to pytest or another framework;
- `integration/ptsip_bridge.py` — the only intended VPMS-facing boundary for reading stable PTSIP target metadata.

`domain`, `execution`, and `integration` are implementation-responsibility boundaries. They are not new VPMS purposes and they are not PTSIP Planes.

A new module MUST be placed under the responsibility that owns its behavior. A new top-level `src/vpms/*.py` module is permitted only when it is a genuine subsystem-wide façade with a distinct responsibility that cannot belong to one of the existing responsibility packages. Unreleased flat import paths are not kept through forwarding wrappers merely for compatibility with branch-internal implementation history.

The flat WU-02 through WU-05 files were reorganized into these responsibility packages by structural refactor commit `971a946f46c7a934fbef3cd24db63a1de59de10c` before further VPMS feature work.
...
### 21.1 Repository test ownership layout

This repository's implementation tests must also expose subsystem and implementation-responsibility ownership physically rather than accumulating at one `tests/` root.
...
tests/
├─ ptsip/
│  └─ existing PTSIP subsystem tests
│
└─ vpms/
   ├─ architecture/
   │  └─ isolation / dependency-direction tests
   ├─ domain/
   │  └─ model / registry / selector semantics
   ├─ execution/
   │  └─ runner / adapter behavior
   └─ integration/
      └─ external-boundary integration such as PTSIP metadata consumption
```

This physical layout serves two purposes:

1. a maintainer can identify whether a test protects PTSIP or VPMS without filename-prefix guessing;
2. implementation responsibilities can be exercised independently, for example `python -m pytest -q tests/vpms/domain`, `python -m pytest -q tests/vpms/execution`, `python -m pytest -q tests/vpms/integration`, or `python -m pytest -q tests/ptsip`.

The full `python -m pytest -q tests` path remains the explicit whole-repository regression path; it is not required merely to inspect one subsystem or one VPMS implementation responsibility.

The physical test path is **not** authority for VPMS verification purpose. For example, `tests/ptsip/` means that PTSIP implementation is the subsystem under test; it does not by itself mean that every contained test is VPMS `PRODUCT` purpose. WU-06 must inspect the correctness responsibility protected by each existing PTSIP test before assigning `PRODUCT`, `TOOLCHAIN`, or an unresolved note. Any finer physical organization of existing PTSIP tests must follow that responsibility inventory rather than replace it.
...
At minimum, `domain/model.py` should represent:
...
`integration/ptsip_bridge.py` exists to make the dependency direction visible and auditable rather than allowing arbitrary VPMS modules to import PTSIP internals.
...
PTSIP stable data
       |
       v
integration/ptsip_bridge.py
       |
       v
VPMS domain / selection
```
...
The first physical move separates **test implementation ownership**, not VPMS verification purpose:

```text
tests/ptsip/
    PTSIP subsystem tests

tests/vpms/
    VPMS subsystem tests, grouped by VPMS implementation responsibility
```

This ownership split was established by structural refactor commit `971a946f46c7a934fbef3cd24db63a1de59de10c`. It allows subsystem/responsibility-focused pytest execution without claiming that directory placement determines VPMS purpose.

Purpose migration still begins with classification by the correctness responsibility each verification protects.
...
The WU-06 semantic inventory is recorded in `planning/0.3.5-vpms-test-inventory.md` from branch baseline `d29ac472ca4939eee62361fe0a082b1bb4409df5` and inventory tree `91cea5f995330f66e8201bafe9bc504b366a9db6`. It covers all 19 existing `tests/ptsip/` modules: 18 are homogeneous `PRODUCT` candidates, while `test_release_readiness_030.py` is intentionally recorded as a mixed physical module whose contained tests resolve to four `PRODUCT` cases and two `TOOLCHAIN` cases. No contained test remains unresolved, and the inventory provides no evidence for a bulk `tests/ptsip/product/` / `tests/ptsip/toolchain/` move.

The transition sequence is:

1. keep PTSIP and VPMS implementation tests physically separated by subsystem ownership;
2. inventory each existing `tests/ptsip/` test and identify the correctness responsibility it protects;
3. assign VPMS purpose from that responsibility, or record an explicit unresolved note;
4. identify genuinely purpose-neutral Formula candidates;
5. separate mutable Variables from governed Policy where current tests mix them;
6. register Verification Cases without requiring an immediate Product/Toolchain directory move of every existing PTSIP test;
7. prove Product-only and Toolchain-only selection;
8. retain subsystem-focused and explicit full-suite regression paths;
9. reorganize existing PTSIP tests more finely only where responsibility evidence makes the grouping unambiguous and the move produces a maintenance benefit.

Directory movement MUST NOT be used as a substitute for semantic classification. Conversely, implementation-responsibility folders such as `vpms/domain` or `vpms/execution` describe code/test ownership and MUST NOT be interpreted as verification-purpose values.
...
| `WU-01` | `S0` | Create independent `src/vpms/` package skeleton and isolation baseline | — | VPMS can be imported/removed without changing PTSIP core behavior; no `ptsip -> vpms` dependency exists | `COMPLETE` | skeleton `52b1fa57d6ac275295416ca1819a53f889281edd`; isolation test `9d12692dfc7ffd7c81fb054ce80cfad6bfca50c1`; isolated VPMS import/syntax smoke PASS; structural refactor `971a946f46c7a934fbef3cd24db63a1de59de10c` moved isolation coverage to `tests/vpms/architecture/test_isolation_035.py` |
| `WU-02` | `S1` | Canonical VPMS model in `domain/model.py`: `VerificationPurpose`, `VerificationCase`, references, `VerificationResult` | `WU-01` | purpose is explicit; Formula/Variables/Policy/Runner identities remain distinct; model tests pass | `COMPLETE` | model commit `577102d1e63fdb5610dabe3e0537e2ebae166ebf`; original focused test -> `6 passed`; structural refactor `971a946f46c7a934fbef3cd24db63a1de59de10c` moved coverage to `tests/vpms/domain/test_model_035.py` |
| `WU-03` | `S2` | Registry and definition validation in `domain/registry.py` | `WU-02` | valid/invalid definitions and unresolved references produce deterministic machine-readable diagnostics | `COMPLETE` | registry commit `552ef340691ea1078f25bb0aa553a8a5c958332e`; original focused test -> `8 passed`; structural refactor `971a946f46c7a934fbef3cd24db63a1de59de10c` moved coverage to `tests/vpms/domain/test_registry_035.py` |
| `WU-04` | `S2` | Framework-neutral runner and normalized result contract in `execution/runner.py` | `WU-02` | runner result preserves case/purpose/target identity and cannot mutate PTSIP classification state | `COMPLETE` | runner commit `fe62b2ce1cc0b55fd49ff8238b201424963bd796`; original focused test -> `7 passed`; committed blobs matched tested blobs; structural refactor `971a946f46c7a934fbef3cd24db63a1de59de10c` moved coverage to `tests/vpms/execution/test_runner_035.py` |
| `WU-05` | `S2` | Read-only PTSIP metadata bridge in `integration/ptsip_bridge.py` | `WU-02` | stable target metadata is consumable through one narrow direction; no authority/profile write path exists | `COMPLETE` | bridge commit `7e1b9ecd6b985a8d1599ba5ef181f767c546969f`; original focused test -> `13 passed`; committed blobs matched tested blobs; structural refactor `971a946f46c7a934fbef3cd24db63a1de59de10c` moved coverage to `tests/vpms/integration/test_ptsip_bridge_035.py` |
| `WU-06` | `S0` | Inventory existing `tests/ptsip/` tests by the correctness responsibility they protect | — | each inventoried test has recorded Product/Toolchain purpose evidence or an explicit unresolved note; physical grouping does not substitute for purpose evidence | `COMPLETE` | inventory commit `0f20519289bf6d3ec21d6117a980b10f11db9d4d`; `planning/0.3.5-vpms-test-inventory.md`; baseline tree `91cea5f995330f66e8201bafe9bc504b366a9db6`; 19 modules covered: 18 homogeneous PRODUCT, 1 mixed module resolved at test level to 4 PRODUCT / 2 TOOLCHAIN, no unresolved test responsibility |
| `WU-07` | `S3` | Formula registration/reuse and purpose-isolation semantics | `WU-03` | Product and Toolchain cases may share a Formula without sharing purpose, Variables, Policy, or result identity | `COMPLETE` | implementation commit `eea2714278387ae9867787461eb740223b52ffdb`; `tests/vpms/domain/test_formula_reuse_035.py` -> 12 passed; exact existing `tests/vpms/domain/test_registry_035.py` plus WU-07 focused suite -> 20 passed; committed implementation/test blobs matched tested blobs |
| `WU-08` | `S3` | Purpose-based selector in `domain/selector.py` | `WU-03` | `PRODUCT`, `TOOLCHAIN`, and explicit full selection are deterministic and path heuristics are not authoritative | `COMPLETE` | implementation commit `4a9bd867b82b9001b9aedaa3401d5d4163b69c11`; `tests/vpms/domain/test_selector_035.py` -> 15 passed; committed selector/test blobs matched locally tested blobs; explicit `SelectionScope.PRODUCT`, `TOOLCHAIN`, `FULL` uses `VerificationCase.purpose` as selection authority and does not infer from path-shaped identity |
| `WU-09` | `S3` | Generic command adapter in `execution/adapters/command.py` | `WU-04` | command-backed cases execute through the normalized runner contract without pytest-specific coupling | `COMPLETE` | implementation commit `1464ece49940ed4d0e8905309f869eee29f230dc`; `tests/vpms/execution/test_command_adapter_035.py` -> 15 passed; exact existing `tests/vpms/execution/test_runner_035.py` plus WU-09 execution suite -> 22 passed; committed adapter/test blobs matched tested blobs; argv execution uses `shell=False`, exit `0 -> PASS`, nonzero -> `FAIL`, and OS/subprocess exceptions -> `ERROR` |
| `WU-10` | `S4` | End-to-end selective execution integration | `WU-07`, `WU-08`, `WU-09` | Product-only, Toolchain-only, and full case sets execute independently while shared Formula use remains non-coupling | `COMPLETE` | implementation commit `4bcbe1212dbdc9c77faacb589594340176b69f1a`; `tests/vpms/execution/test_selective_execution_035.py`; local selective-execution suite -> 7 passed; existing runner regression plus WU-10 candidate -> 14 passed; Product-only and Toolchain-only execution require only their selected runner registrations; full execution preserves independent outcomes and shared-Formula non-coupling |
| `WU-11` | `S4` | PTSIP/VPMS boundary integration tests | `WU-05`, `WU-10` | tests prove no `ptsip -> vpms` runtime dependency, no authority mutation, no classification mutation, and no conformance/result collapse | `COMPLETE` | test commit `81490e4bdabd2f7f6c5265edc9ac361b7e6d6e6a`; `tests/vpms/integration/test_ptsip_boundary_035.py` -> 2 passed; exact committed test/model/runner/bridge/DecisionStore blobs matched tested blobs; post-WU-10 repository regression at `9b550320261b66bb2250ac1818ce5c199e3f4b0d` -> 225 passed and source/test implementation remained unchanged through WU-11 start; boundary execution preserves PTSIP profile bytes, TOOLCHAIN classification metadata, and Decision Authority record while a PRODUCT-purpose VPMS case passes; VPMS execution outcomes remain distinct from PTSIP conformance outcomes |
| `WU-12` | `S5` | Repository self-adoption: register representative real Verification Cases from the existing test inventory | `WU-03`, `WU-05`, `WU-06`, `WU-10` | real repository Product-purpose and Toolchain-purpose subsets are demonstrated; full regression path remains available | `COMPLETE` | naming refinement commit `49f2d58555eb6c9219d09f0362aa1ef4952e514c`; `tests/vpms/integration/test_repository_self_adoption_035.py`; exact-blob focused self-adoption suite -> 6 passed in 0.08s; test/model/registry/selector/runner/command and mixed PTSIP test blobs matched the committed blobs; four real test-level cases from mixed `tests/ptsip/test_release_readiness_030.py` remain registered as 2 PRODUCT + 2 TOOLCHAIN cases; both purposes share the purpose-neutral `command.exit-zero` Formula; Policy identifiers express governed responsibility as `distribution.contract-integrity`, `distribution.package-integrity`, `release.workflow-integrity`, and `ci.verification-boundary` without encoding PRODUCT/TOOLCHAIN or pytest; PRODUCT, TOOLCHAIN, and FULL scopes map deterministically to real pytest node IDs through the generic command adapter; `python -m pytest -q tests` remains a separate explicit full-regression path |
| `WU-13` | `S6` | Update `adoption/ADOPTION-GUIDE.md` from implemented VPMS behavior | `WU-12` | every documented VPMS behavior has an implemented/tested counterpart and the reference layout remains optional | `COMPLETE` | documentation commit `832f978186d7f4d6ce198aadfbebfd1e8f523ed9`; `adoption/ADOPTION-GUIDE.md` adds the optional VPMS adoption section with 194 additions and no deletions to the existing PTSIP phases; documentation was cross-checked against ADR-0006 and implemented WU-07 through WU-12 semantics; it records PRODUCT/TOOLCHAIN purpose separation, Verification Case composition, purpose-neutral Formula reuse, responsibility-oriented Policy naming, optional reference layout, PRODUCT/TOOLCHAIN/FULL selection, framework-neutral command execution, one-way read-only PTSIP metadata consumption, and VPMS-result/conformance separation; no unimplemented VPMS CLI command is documented |
| `WU-14` | `S6` | Update `agents/AGENT-CONTRACT.md` with VPMS purpose-first obligations | `WU-03`, `WU-10`, `WU-12` | agent rules bind to real VPMS metadata/diagnostics; no path-only purpose inference or silent Policy repair is permitted | `COMPLETE` | agent-contract commit `3d193242efeaa8b713e31e3671bf57dc2a6484c8`; `agents/AGENT-CONTRACT.md` adds VPMS Section 17 with 60 additions and no deletions to the existing PTSIP contract; obligations bind purpose decisions to `VerificationCase.purpose`, preserve Formula/Variables/Policy/Runner distinctions, forbid path/framework-only inference and silent Policy repair, preserve PRODUCT/TOOLCHAIN selection separation, and keep VPMS results separate from PTSIP conformance; Registry guidance binds to `vpms-registry-diagnostic/v1` and the implemented diagnostic codes including `UNKNOWN_PURPOSE`, `UNRESOLVED_REFERENCE`, and `DUPLICATE_CASE_ID`; Runner guidance preserves `VPMS-RUN-CONTRACT-ERROR` / `VPMS-RUN-EXECUTION-ERROR` as VPMS execution diagnostics rather than PTSIP decisions |
| `WU-15` | `S2` | Packaging inclusion for sibling `vpms` package and responsibility subpackages | `WU-01` | source distribution and wheel include VPMS responsibility packages without changing existing PTSIP command behavior | `COMPLETE` | implementation commit `f4204eb513be6f8a593522a7d1d6d6f396e94dbb`; `pyproject.toml` explicitly discovers `ptsip*` and `vpms*`; `tests/vpms/architecture/test_packaging_035.py` candidate -> 2 passed; committed pyproject/test blobs matched the candidate; setuptools `build_meta` packaging-topology probe using the committed discovery config and current VPMS file topology generated sdist and wheel containing all 11 current VPMS Python files while preserving `ptsip` / `ptsip-app` entry points and adding no VPMS script; final exact-checkout build and installed-wheel verification remain `WU-18` |
| `WU-16` | `S6` | Decide and implement the minimum required public/CLI surface | `WU-10`, `WU-12` | public surface reflects proven VPMS semantics and introduces no unnecessary PTSIP-core coupling | `COMPLETE` | implementation commit `d5393df6cd118b0c1176afc8310d07b59aedb4b2`; `src/vpms/__init__.py` establishes a curated Python facade for proven model/Registry/Selector/Runner/CommandExecutor contracts; `tests/vpms/architecture/test_public_surface_035.py` plus updated isolation candidate -> 7 passed in staged import-topology verification; PTSIP integration remains explicit under `vpms.integration` and is not flattened into the package root; `pyproject.toml` contains no `vpms` console script; no PTSIP-core dependency was introduced; a VPMS CLI is intentionally deferred because Tool 0.3.5 has no proven persisted Verification Definition / executor CLI input contract |
| `WU-17` | `S7` | Tool `0.3.5` version/identity stabilization | `WU-00`, `WU-11`, `WU-13`, `WU-14`, `WU-15`, `WU-16` | delivered VPMS surface, documentation, packaging, and governance are stable enough for the `0.3.5` identity | `COMPLETE` | identity commit `2c0b8151f44218e3c7e9110cdb7858bf17d29484`; `pyproject.toml` project version and runtime `TOOL_VERSION` are `0.3.5`; `ptsip.__version__` now derives from `TOOL_VERSION`; `tests/ptsip/test_tool_identity_035.py` exact-blob focused suite -> 2 passed in 0.07s; Specification binding remains `0.3.4-draft @ b5b17dd16667cc1afaf1d23054b6e5dd773e3f5e`; WU-18 discovery aligned the final conformance-report version expectation in commit `06b2db8078da57a11a2cd59059df6b74cdd5b208` |
| `WU-18` | `S8` | Full regression, build, installed-wheel, and implementation-completion verification | `WU-17` | all Section 30 acceptance criteria are reproducibly verified; release note remains deferred until explicit release preparation | `COMPLETE` | final GitHub Actions `tooling-test` run `31930110185`, job `95123510309`, Python `3.14.7`: `240 passed in 10.25s`; source identity/VPMS public-surface smoke PASS; `ptsip --version`, `ptsip spec`, and `ptsip conform --help` PASS; `python -m build` produced `ptsip-0.3.5.tar.gz` and `ptsip-0.3.5-py3-none-any.whl`; `twine check` PASS for both; sdist/wheel topology check confirmed all 11 current VPMS Python files; built wheel force-reinstall PASS; installed-wheel VPMS import from `site-packages` and Tool/Specification identity smoke PASS. WU-18 exposed and corrected a missing dev `setuptools` dependency (`fc1f11bdd161880b5e778cf620b0f101af437a69`) and one stale Tool-version expectation (`06b2db8078da57a11a2cd59059df6b74cdd5b208`) before the final successful run. The one-off verification branch used the same `pyproject.toml` and conformance-test blob SHAs as the implementation branch; its extra surface was only the temporary workflow trigger/final-gate steps |
| `WU-19` | `S9` | Align PTSIP install/upgrade guidance and the PyPI project-description source | — | `README.md` and `README.ko.md` clearly distinguish new installation (`python -m pip install PTSIP`) from upgrade (`python -m pip install --upgrade PTSIP`); `README.md`, which is the configured project readme, carries the same guidance into the next distribution's PyPI long description; work is complete before branch merge | `COMPLETE` | documentation commit `09524e6b37aaa89b237420f318f881a6a129dbdb`; canonical `README.md` already satisfied the gate at WU-19 start (blob `47bcfe9d6e2561b39854c328302aad09d5a9401b`) with explicit new-install and upgrade commands; `README.ko.md` was aligned to the same commands (blob `ec4594c13d00aa4826de908ccaf0e8e4956f49c0`); `pyproject.toml` blob `6b83a3d945d7977fba728c667424e05775ea7e83` retains `[project] readme = "README.md"`, so the next uploaded distribution carries the same guidance into PyPI long-description metadata; no release or publication was performed |
...
All defined Tool `0.3.5` Work Units (`WU-00` through `WU-19`) are now `COMPLETE`. The implementation-completion gate and the mandatory pre-merge installation/upgrade guidance gate have both passed. Explicit release preparation remains a separate boundary, and `releasenote/0.3.5.md` remains deferred until that boundary begins.
...
1. `src/vpms/` exists as an independently owned sibling package with explicit `domain`, `execution`, and `integration` responsibility boundaries;
...
13. repository implementation tests expose PTSIP/VPMS ownership separately and VPMS tests can be exercised by implementation responsibility without requiring the whole repository suite;
14. representative existing repository tests are classified and exercised through VPMS semantics;
15. `adoption/ADOPTION-GUIDE.md` reflects implemented VPMS adoption behavior;
16. `agents/AGENT-CONTRACT.md` reflects implemented VPMS agent obligations;
17. ADR-0006 records the VPMS architecture/governance decision without rewriting earlier ADR history;
18. existing PTSIP regressions continue to pass;
19. build and installed-wheel verification include the sibling VPMS responsibility packages without breaking existing PTSIP commands;
20. no unrelated PTSIP authority/coordination expansion is mixed into the VPMS implementation;
21. `releasenote/0.3.5.md` is created only when explicit release preparation begins.

All 21 criteria are satisfied by the accumulated WU-00 through WU-18 evidence. Criterion 21 remains satisfied specifically because no `releasenote/0.3.5.md` exists during implementation completion; explicit release preparation has not begun.
...
WU-19 completion evidence:

- canonical `README.md` blob `47bcfe9d6e2561b39854c328302aad09d5a9401b` explicitly distinguishes new installation from upgrade using the required `python -m pip` commands;
- `README.ko.md` blob `ec4594c13d00aa4826de908ccaf0e8e4956f49c0` communicates the same distinction in Korean with byte-for-byte identical command lines;
- `pyproject.toml` blob `6b83a3d945d7977fba728c667424e05775ea7e83` continues to configure `readme = "README.md"` for distribution/PyPI long-description metadata;
- the documentation alignment commit is `09524e6b37aaa89b237420f318f881a6a129dbdb`; no PyPI upload, Tool tag, GitHub Release, or release-note creation is part of WU-19.

## 32. Interim repository regression evidence

A full repository pytest regression was executed locally after `WU-10` against the exact branch state below. This evidence records an implementation checkpoint and is retained for history; it has now been superseded as the final implementation-completion gate by Section 33.

```text
branch: tool-0.3.5-vpms
commit: 9b550320261b66bb2250ac1818ce5c199e3f4b0d
command: python -m pytest -q tests
Python: 3.14.6
pytest: 8.4.2
result: 225 passed, 0 failed, 0 skipped
exit code: 0
duration: 220.79 seconds
working tree before/after: clean
```

The repository `src` directory was prepended to `PYTHONPATH` so the checkout under test took precedence over a stale user-site installation. Bytecode and pytest cache writes were disabled, while external pytest plugin autoload remained enabled.

A non-fatal Windows `WinError 5` occurred only during pytest exit-time cleanup of the local `pytest-current` temporary-directory reparse point. It did not affect any test outcome or the final exit code and is recorded as a local environment/test-infrastructure note rather than a repository regression failure.

This checkpoint demonstrates that the post-`WU-10` branch state retained whole-repository pytest compatibility at commit `9b550320261b66bb2250ac1818ce5c199e3f4b0d` without changing migration state, Work Unit status, or release readiness claims.

## 33. Final WU-18 implementation-completion verification

The final Tool `0.3.5` implementation gate was executed through one-off GitHub Actions verification branch `wu18-verify-035` to obtain an exact clean-checkout Linux/Python build and installed-wheel boundary without creating a Tool tag or invoking the release/publish workflow.

The first two verification attempts usefully exposed two implementation-readiness defects before the final gate:

1. run `31929921538` failed during collection because `tests/vpms/architecture/test_packaging_035.py` imports setuptools while the `dev` extra did not declare it; commit `fc1f11bdd161880b5e778cf620b0f101af437a69` added `setuptools>=77` to the development verification environment;
2. run `31929976406` reached `239 passed / 1 failed` and exposed one stale Tool-version assertion in `tests/ptsip/test_conformance_030.py`; commit `06b2db8078da57a11a2cd59059df6b74cdd5b208` aligned that report expectation to Tool `0.3.5` while retaining all `0.3.4-draft` Specification fixtures.
...
run id: 31930110185
job id: 95123510309
verification branch head: 2d8cd6b07c3b825589c64ef89262bb05274dbac0
Python: 3.14.7
pytest: 8.4.2
command: python -m pytest -q
result: 240 passed in 10.25s

source/editable identity:
    distribution version = 0.3.5
    TOOL_VERSION = 0.3.5
    ptsip.__version__ = 0.3.5
    Specification = 0.3.4-draft
    Specification revision = b5b17dd16667cc1afaf1d23054b6e5dd773e3f5e

build:
    ptsip-0.3.5.tar.gz
    ptsip-0.3.5-py3-none-any.whl
    twine check = PASS for both

VPMS distribution boundary:
    all 11 current VPMS Python files present in sdist and wheel
    built wheel force-reinstall = PASS
    installed vpms import resolved from site-packages
    PRODUCT / TOOLCHAIN purpose identities available after installation

PTSIP CLI boundary before and after wheel reinstall:
    ptsip --version -> PTSIP Tool 0.3.5
    ptsip spec -> 0.3.4-draft @ b5b17dd16667cc1afaf1d23054b6e5dd773e3f5e, tool_version 0.3.5
    ptsip conform --help -> PASS
```

The final verification branch carried the same `pyproject.toml` blob (`6b83a3d945d7977fba728c667424e05775ea7e83`) and final conformance-test blob (`ebf0c319e499c74252de2166312c6c66dc657759`) as the implementation branch. All other implementation content was inherited from the common Tool `0.3.5` baseline; the verification branch's intentional difference was the temporary `tooling-test.yml` branch trigger and additional final-gate smoke steps. No release tag, GitHub Tool Release, PyPI publication, or `releasenote/0.3.5.md` was created by this verification.
