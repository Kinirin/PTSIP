...
| `WU-15` | `S2` | Packaging inclusion for sibling `vpms` package and responsibility subpackages | `WU-01` | source distribution and wheel include VPMS responsibility packages without changing existing PTSIP command behavior | `READY` | — |
| `WU-16` | `S6` | Decide and implement the minimum required public/CLI surface | `WU-10`, `WU-12` | public surface reflects proven VPMS semantics and introduces no unnecessary PTSIP-core coupling | `READY` | — |
...
WU-16  minimum public / CLI surface
WU-19  install / upgrade + PyPI description alignment
```

`WU-06`, `WU-07`, `WU-08`, `WU-09`, `WU-10`, `WU-11`, `WU-12`, `WU-13`, and `WU-14` are complete. WU-14 completion removes another dependency from `WU-17`; `WU-17` still has unmet dependencies in `WU-15` and `WU-16`, so it remains `PLANNED`. `WU-15`, `WU-16`, and `WU-19` are immediately eligible, and their relative order is not an architectural dependency.
