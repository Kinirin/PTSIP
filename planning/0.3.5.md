...
| `WU-12` | `S5` | Repository self-adoption: register representative real Verification Cases from the existing test inventory | `WU-03`, `WU-05`, `WU-06`, `WU-10` | real repository Product-purpose and Toolchain-purpose subsets are demonstrated; full regression path remains available | `COMPLETE` | final self-adoption tree through commit `e94cc732d4792559773749a8669e0ef8c8076e65`; `tests/vpms/integration/test_repository_self_adoption_035.py`; focused local self-adoption suite -> 6 passed; four real test-level cases from mixed `tests/ptsip/test_release_readiness_030.py` are registered as 2 PRODUCT + 2 TOOLCHAIN cases; both purposes share the purpose-neutral `pytest.node-exit-zero` Formula while retaining purpose-specific Policy; PRODUCT, TOOLCHAIN, and FULL scopes map deterministically to real pytest node IDs through the generic command adapter; `python -m pytest -q tests` remains a separate explicit full-regression path |
