...
| `WU-09` | `S3` | Generic command adapter in `execution/adapters/command.py` | `WU-04` | command-backed cases execute through the normalized runner contract without pytest-specific coupling | `READY` | — |
...
`WU-06`, `WU-07`, and `WU-08` are complete. `WU-09` remains eligible from `WU-04`; `WU-15` remains eligible from `WU-01`; `WU-19` remains an independent root. `WU-10` now has `WU-07` and `WU-08` satisfied and remains gated only by `WU-09`. `WU-12` has `WU-03`, `WU-05`, and `WU-06` satisfied and remains gated only by `WU-10`. Their relative order is not an architectural dependency.
