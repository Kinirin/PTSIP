...
| `WU-01` | `S0` | Create independent `src/vpms/` package skeleton and isolation baseline | — | VPMS can be imported/removed without changing PTSIP core behavior; no `ptsip -> vpms` dependency exists | `READY` | — |
| `WU-02` | `S1` | Canonical VPMS model: `VerificationPurpose`, `VerificationCase`, references, `VerificationResult` | `WU-01` | purpose is explicit; Formula/Variables/Policy/Runner identities remain distinct; model tests pass | `PLANNED` | — |
| `WU-03` | `S2` | Registry and definition validation in `registry.py` | `WU-02` | valid/invalid definitions and unresolved references produce deterministic machine-readable diagnostics | `PLANNED` | — |
| `WU-04` | `S2` | Framework-neutral runner and normalized result contract in `runner.py` | `WU-02` | runner result preserves case/purpose/target identity and cannot mutate PTSIP classification state | `PLANNED` | — |
| `WU-05` | `S2` | Read-only PTSIP metadata bridge in `ptsip_bridge.py` | `WU-02` | stable target metadata is consumable through one narrow direction; no authority/profile write path exists | `PLANNED` | — |
...
| `WU-07` | `S3` | Formula registration/reuse and purpose-isolation semantics | `WU-03` | Product and Toolchain cases may share a Formula without sharing purpose, Variables, Policy, or result identity | `PLANNED` | — |
| `WU-08` | `S3` | Purpose-based selector in `selector.py` | `WU-03` | `PRODUCT`, `TOOLCHAIN`, and explicit full selection are deterministic and path heuristics are not authoritative | `PLANNED` | — |
| `WU-09` | `S3` | Generic command adapter in `adapters/command.py` | `WU-04` | command-backed cases execute through the normalized runner contract without pytest-specific coupling | `PLANNED` | — |
...
| `WU-15` | `S2` | Packaging inclusion for sibling `vpms` package | `WU-01` | source distribution and wheel include VPMS without changing existing PTSIP command behavior | `PLANNED` | — |
...
| `WU-19` | `S9` | Align README installation and upgrade guidance for PTSIP users | `WU-18` | `README.md` and `README.ko.md` clearly distinguish new installation (`python -m pip install PTSIP`) from upgrade (`python -m pip install --upgrade PTSIP`); the documentation-only change does not trigger a standalone release and is carried into the next explicit release preparation | `PLANNED` | — |
...
| `S9` | Post-verification user installation guidance | `WU-19` | Documentation-only follow-up after `WU-18`; keep release preparation and release-note creation separate |
...
flowchart TD
    ...
    WU18 --> WU19["WU-19 README install / upgrade guidance"]
```
...
At the current planning baseline, the immediately eligible independent roots are:

```text
WU-00  ADR-0006 governance record
WU-01  VPMS package skeleton / isolation baseline
WU-06  existing test inventory
```

These may be implemented together or in separate sessions. Their relative order is not an architectural dependency.
...
## 31. Post-WU-18 README installation/upgrade guidance

`WU-19` is intentionally sequenced after `WU-18`. It is a user-facing documentation follow-up, not an additional VPMS implementation-completion criterion, so it does not create a dependency cycle with Section 30.

The README installation guidance must distinguish the two user intents explicitly:
...
`README.ko.md` should communicate the same distinction in Korean while preserving the same commands. This change does not by itself justify a standalone PyPI release. It must be present before the next explicit release preparation so the subsequently built distribution metadata carries the updated README guidance.
