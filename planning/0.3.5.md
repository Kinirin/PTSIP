...
`WU-06` is complete from the recorded semantic inventory. `WU-09` became eligible when `WU-04` completed. `WU-07` and `WU-08` remain eligible from `WU-03`; `WU-15` remains eligible from `WU-01`; `WU-19` remains an independent root. `WU-12` now has `WU-03`, `WU-05`, and `WU-06` satisfied and remains gated only by `WU-10`. Their relative order is not an architectural dependency.
