...
| `WU-08` | `S3` | Purpose-based selector in `domain/selector.py` | `WU-03` | `PRODUCT`, `TOOLCHAIN`, and explicit full selection are deterministic and path heuristics are not authoritative | `READY` | — |
...
`WU-06` and `WU-07` are complete. `WU-08` remains eligible from `WU-03`; `WU-09` remains eligible from `WU-04`; `WU-15` remains eligible from `WU-01`; `WU-19` remains an independent root. `WU-10` now has `WU-07` satisfied and remains gated by `WU-08` and `WU-09`. `WU-12` has `WU-03`, `WU-05`, and `WU-06` satisfied and remains gated only by `WU-10`. Their relative order is not an architectural dependency.
