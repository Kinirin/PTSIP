...
def _repo(tmp_path: Path) -> Path:
    repo = tmp_path / "repo"
    (repo / "developer/planning/0.4.0/WU-04").mkdir(parents=True)
    (repo / "developer/planning/0.4.0/WU-05").mkdir(parents=True)
    (repo / "pyproject.toml").write_text("[project]\nname='fixture'\nversion='0.0.0'\n", encoding="utf-8")
    (repo / "developer/planning/index.yaml").write_text(ROOT_INDEX, encoding="utf-8")
    (repo / "developer/planning/0.4.0/index.yaml").write_text("plan: fixture\n", encoding="utf-8")
    (repo / "developer/planning/0.4.0/WU-04/WU-04.yaml").write_text("work_unit: WU-04\n", encoding="utf-8")
    (repo / "developer/planning/0.4.0/WU-05/WU-05.yaml").write_text("work_unit: WU-05\n", encoding="utf-8")
    return repo
...
@pytest.mark.parametrize(
    ...
    [
        ("dev/0.4.0", "developer/planning/0.4.0/index.yaml", None, "INTEGRATION_CONTROL_PLANE"),
        (
            "dev/0.4.0-WU-04",
            "developer/planning/0.4.0/WU-04/WU-04.yaml",
            "WU-04",
            "INDEPENDENT_LEAF",
        ),
        (
            "dev/0.4.0-WU-05",
            "developer/planning/0.4.0/WU-05/WU-05.yaml",
            "WU-05",
            "INDEPENDENT_LEAF",
        ),
    ],
...
def test_duplicate_exact_branch_fails_closed(tmp_path: Path):
    repo = _repo(tmp_path)
    root_index = repo / "developer/planning/index.yaml"
...
def test_missing_entry_document_fails_closed(tmp_path: Path):
    repo = _repo(tmp_path)
    (repo / "developer/planning/0.4.0/WU-05/WU-05.yaml").unlink()
...
def test_cli_path_only_is_machine_consumable(tmp_path: Path, capsys):
    ...
    assert capsys.readouterr().out.strip() == "developer/planning/0.4.0/WU-04/WU-04.yaml"
