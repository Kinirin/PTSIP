...
The tagged Tool note was finalized immediately before publication. Current operational status corrections belong in this index and `.ptsip/state/current.json` rather than rewriting an immutable tagged release point.
...
- record post-publication operational corrections in `releasenote/README.md` and `.ptsip/state/current.json`;
