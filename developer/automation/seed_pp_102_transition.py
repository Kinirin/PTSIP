...
def _write_pp_102_acceptance_test(repo: Path, changed: list[str]) -> None:
    path = repo / "tests" / "ptsip" / "identity" / "test_pp_102_current_contract.py"
...
def seed(root: str | Path | None = None) -> tuple[str, ...]:
    ...
    _write_pp_102_acceptance_test(repo, changed)
